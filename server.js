...
try {
  ...
  app.use(
    "/api-docs/users",
    swaggerUi.serveFiles(usersOpenapi, {}),
    swaggerUi.setup(usersOpenapi),
  );
} catch (e) {
...
try {
  ...
  app.use(
    "/api-docs/tenants",
    swaggerUi.serveFiles(tenantsOpenapi, {}),
    swaggerUi.setup(tenantsOpenapi),
  );
} catch (e) {
...
try {
  ...
  app.use(
    "/api-docs/refund-rules",
    swaggerUi.serveFiles(rrOpenapi, {}),
    swaggerUi.setup(rrOpenapi),
  );
} catch (e) {
...
try {
  ...
  app.use(
    "/api-docs/refunds",
    swaggerUi.serveFiles(refundsOpenapi, {}),
    swaggerUi.setup(refundsOpenapi),
  );
} catch (e) {
...
try {
  const refStatsSpecPath = path.join(
    __dirname,
    "docs",
    "openapi.refund-stats.json",
  );
  const refStatsOpenapi = JSON.parse(
    fs.readFileSync(refStatsSpecPath, "utf-8"),
  );
  app.get("/openapi.refund-stats.json", (req, res) =>
    res.json(refStatsOpenapi),
  );
  app.use(
    "/api-docs/refund-stats",
    swaggerUi.serveFiles(refStatsOpenapi, {}),
    swaggerUi.setup(refStatsOpenapi),
  );
} catch (e) {
...
try {
  ...
  app.use(
    "/api-docs/user-audit",
    swaggerUi.serveFiles(uaOpenapi, {}),
    swaggerUi.setup(uaOpenapi),
  );
} catch (e) {
